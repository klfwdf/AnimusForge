# AnimusForge Xihai Action / SceneActions v1.1
...
关键词，也不调用轻量通道分类器；`他人演讲` 分支的输入是主题/要求，由 AF
为当前框选主目标 NPC 重新生成一段面向士兵的正文。Y 菜单只在
...
- 被认领的 NPC 回复会追加优先级更高的阵前演讲专用提示词：只允许输出面向己方将士的鼓舞正文，禁止星号动作、旁白、向玩家请示或自贬拒绝；MCM 可在 6-30 字内设置最少和最多字数。模型仍越界时按完整句校验并回退到合规号召，不截断半句话。
...
- MCM 页面名为“自然语言动作与阵前演讲”，保留27项动作、NPC站位/踱步、听众、声音、推进、安全半径、通知和诊断设置；原先零散的玩家/NPC自然动作和AF分类选项合并为一个“自然语言回复动作”开关并附说明。运行时覆盖再次验证，非法组合失败关闭。
...
- 本轮不修改 AF 主工程、不覆盖 AF DLL、不把 SceneActions 整合进 AF；未来 Typed Bridge 可以在不改动作核心和 Mission 执行器的前提下替换当前反射桥。
...
带冒号的强制命令会在 AF 普通场景喊话入口之前被本模组认领，不再让 SceneActions 普通动作链重复消费。未写人称或写“我”时，发起者（玩家）直接发表冒号后的正文；写“你/他/她/目标”时，当前框选主目标使用冒号后的内容作为主题/要求，由 AF 重新生成一段 NPC 演讲正文：
...
支持 `你/他/她来演讲`、`你/他/她演讲` 以及 `目标演讲` 的同义写法，也支持 ASCII `:` 和中文 `：`。`你/他/她/目标演讲：XXXX` 中的 `XXXX` 不会直接播放，而是作为 AF 的主题要求；AF 只生成一次新的、面向士兵的正文，扩展随后只播放这一次结果。强制命令必须带冒号；缺少冒号时不会进入强制通道，避免误伤普通聊天。明确写 NPC 人称但没有框选主目标时，命令会安全失败，不会改由玩家代演。旧的 `我阵前演讲：` 玩家接口保持不变。
...
框架会先检查是否以“弟兄们/将士们/战士们/全军”等部队称呼开头，并同时包含家园、敌人、阵线、冲锋、胜利等战前修辞；高置信度时本地直接进入 `PLAYER_SPEECH`，整句原文立即播放。较弱但可能是演讲的文本才调用一次轻量闭集分类器；它只返回 `PLAYER_SPEECH`、`NPC_SPEECH`、`ORDINARY_SCENE` 或 `NONE`，不生成正文。`ORDINARY_SCENE` 和超时/非法结果都回到 AF 原普通通道。普通寒暄、引用他人、复述、假设和没有战前修辞的“弟兄们”不会触发。
