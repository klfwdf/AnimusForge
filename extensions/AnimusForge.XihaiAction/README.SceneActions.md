...
- 步兵与骑乘 NPC 都可演讲；NPC先到己方阵线前方并持续面向有效听众。左右横向踱步目前默认关闭，不会向敌军纵深移动、强制下马或传送；站位距离默认 10 米、到位半径 1.5 米、移动超时 15 秒。
...
- NPC 演讲 Claim 创建时冻结一份轻量战场事实快照（同侧/敌侧当前有效人类人数、模块记录的减员、近敌状态、是否交战、战斗类型和阶段），作为普通上下文注入同一次 AF 提示词；程序不输出主题结论、不新增 `THEME`/`TONE` 协议，也不发起额外 AF 请求。AF 仍根据事实自行选择悲壮、冷静、坚定、鼓舞等主风格；明确写在请求中的文风优先。快照没有提供的地形、天气和阵线细节继续由 AF 当前场景上下文提供，禁止臆造。
- 完整演讲正文由 AF 一次闭集调用同时判断最多四个 V4 动作，并生成最多 8 条简短士兵回应；模型不能改变演员、听众、权限、最终开战命令或输出任意 `act_*`。
- 听众冻结为全体同侧有效士兵；实际默认最多 60 人分批播放 `agree/greet/cheer`，每批 6 人、每 Tick 最多提交 6 项。默认 24 名不同士兵生成短口头回应，逻辑波次上限 5 人、每 Tick 最多提交 8 项；随后 22 人分批发出原生 `Victory/Yell`。三类人数、波次和提交预算均可在 MCM 独立调整。
...
- MCM 页面名为 AF 主体的“AnimusForge设置”，分组“自然语言动作与阵前演讲”，保留27项动作、NPC站位、听众、声音、推进、安全半径、通知和诊断设置；“自然语言回复动作”只控制普通 AF 对话/场景喊话回复中的动作解析与播放，“启用阵前演讲”独立控制 T/Y 演讲及其听众回应、战吼和演讲后发令。运行时覆盖再次验证，非法组合失败关闭。
- `af_xihai_daimao_head` 的显示名为“呆喵”，头部防护为 100，XML 标记为非商品；AF 会在会话启动时执行一次生成门：已有实例则只记录并锁定，没有实例则放入一个城镇商店并持久化“已生成”状态。不会每日扫描、删除重复件或在每次商店刷新时重新生成。
...
- 默认确定性选择 24 名不同士兵，分成小波次说出与正文相关的短回应；AF 在同一次演讲语义调用中生成这些短句，运行时复用原 `ShowNpcSpeechOutput` 气泡/TTS/口型入口，不为每名士兵单独请求模型。正文完成前不会开放听众通道；“听众回应开始延迟”默认 3 秒作为最短门槛。MCM 可设置 0–100 人、每波 2–20 人、每 Tick 2–20 项、每条 4–80 字及 0.1–0.5 秒随机波次间隔。
