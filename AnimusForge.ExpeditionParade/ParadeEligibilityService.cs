...
internal static class ParadeEligibilityService
{
	internal static bool CanStartParade(Settlement settlement, Hero player, out TextObject reason)
	{
		...
		{
			reason = Text("村庄正在被劫掠、已被洗劫或尚未恢复，不能举行阅兵。");
			return false;
		}
		...
		if (Mission.Current != null || CampaignMission.Current != null)
		{
			reason = Text("当前已有 Mission 或互斥场景活动。");
			return false;
		}
		...
		if (PlayerEncounter.LocationEncounter == null || ExpeditionParadeCampaignBehavior.ResolveParadeLocation(settlement) == null)
		{
			reason = Text("当前聚落没有可进入的街道、城堡或村庄中心场景。");
			return false;
		}

		ParadeRosterSnapshot snapshot = ParadeRosterSnapshot.Capture(MobileParty.MainParty.MemberRoster, ExpeditionParadeCampaignBehavior.DefaultDisplayLimit);
		if (snapshot.TotalHealthyCount <= 0)
		{
			reason = Text("玩家主队伍中没有可展示的健康普通士兵。");
			return false;
		}

		reason = Text("阶段 0：进入聚落场景，记录导航网格、门口、边界和路线锚点；不会生成或修改士兵。");
		return true;
	}
