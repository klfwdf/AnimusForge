...
> J06 差分补强（2026-09-19，J10 路径更新）：`run.py` 从 `77a3d234` 的根文件和当前 `Channels/Courier/CourierDeliveryBehavior.PromptMessages.cs` 分别提取回信/主动来信最终消息构建器及其历史消息转换 helpers，先断言双方源码相同，再实际编译运行。76 场景比较完整 request JSON，并检查 system、上下文/历史 user、当前信件 user 的顺序；正常 550 checks PASS，Lore／实体／规则三项仅新侧文本丢失变异拒收。Hero/规则/共享上下文输入仍为 fixture，不代表实机/provider 验收。
