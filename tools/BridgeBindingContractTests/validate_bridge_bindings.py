...
def load_json(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        require(size <= MAX_MANIFEST_BYTES, f"{label} exceeds the bounded JSON size")
...
def run(project_root: Path, manifest_path: Path | None = None, catalog_path: Path | None = None) -> dict[str, Any]:
...
def main() -> int:
    ...
    try:
        result = run(args.project_root, args.manifest, args.catalog)
