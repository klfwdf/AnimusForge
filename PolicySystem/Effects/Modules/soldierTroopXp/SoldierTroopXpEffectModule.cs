	...
	: PolicyEffectModuleBase<SoldierTroopXpPayload>, IPolicyEffectCompositeModule
{
	...
	private static readonly PolicyEffectModuleDescriptor ModuleDescriptor = new PolicyEffectModuleDescriptor(
		...
		editableUnderstandingPrompt: "士兵精锐化表示政策通过训练、整编、激励、军纪、福利、军事改革或其他合理机制，让目标家族当前全部正式领主队伍以及该家族城镇、城堡驻军中的普通士兵获得原版兵种经验。玩家主部队在玩家家族成为目标时也属于正式领主队伍。它既可以描述政策生效时集中完成的训练成果，也可以描述政策有效期间持续发生的训练收益；只有正文同时明确两种安排时才同时采用。泛称提高精锐程度而未说明常设制度，通常理解为集中训练成果；明确设立常设教官团、永久训练制度或固定训练营时，理解为持续训练收益。它不增加英雄技能，不作用于俘虏、民兵、商队、村民、强盗、信使或临时队伍，也不直接替换兵种、升级兵种、招募或增加士兵数量。尚未招募的要人志愿兵槽位、志愿兵补充与成长属于另一项；只有政策另行明确让现役领主部队或驻军普通士兵获得经验时，才同时采用士兵精锐化。",
