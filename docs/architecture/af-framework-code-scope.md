...
Courier 最终 commit、入站 receipt、双向 generation、session/route transport、delivery lifetime 分别位于 `Channels/Courier` 的稳定 partial；同步 lookup/载档 reset/runtime index/normalize 位于 `SessionRegistry`，公开 flow、出站 UI/payload 和三类入站 session 建立位于 `SessionCreation`，原责任均未复制。其余主动来信扫描/候选、信件库存、Prompt message/fact、旧 domain commit/wait、B1预算、双向服务/外部三渠道SDK仍未完成；不把某个 partial 或目录 Ready 当整项完成。
