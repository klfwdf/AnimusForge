...
| `memory.summary.planner` | `MyBehavior.cs:5037-5040` | `private async Task ProcessMemorySummaryQueueAsync(bool forceOverviewCandidateScan = false)` — 分段初筛/extra/cleanup，冻结metadata排序与失败汇总在worker；仅完整接受才计数 | `mixed-host` |
| `memory.summary.maintenance` | `MyBehavior.cs:17873-17876` | `private void TryRunCampaignMemoryMaintenance()` — 不在每Tick重复读完整summary来源；past draft检查仍有全扫 | `mixed-host` |
