...
独立外部消费者 55、群组回执 18、Scene 生命周期 34、后处理 37、ChannelCutover 133、NativeCompletion 186、V1 142 与四实际 DLL metadata 1252、Debug/Release × 1.3/1.4 + Bootstrap 六构建通过；[代码地图](af-framework-code-map.json)绑定 `79fa7c48`、730 锚点两模式通过。当前有限状态 `J14_G0_BASELINE_VERIFIED / J14a_OFFLINE_VERIFIED / J14_ACTIVE`，下一包 J14b Courier。真实两版本游戏、旧档、provider、音频、独立子 MOD 游戏加载与帧性能均 `NOT-RUN`，不推导 J14/发布完成；详细测试身份和旧 runner 限制见[主台账](../animusforge-refactoring-and-repository-reorganization-plan.md)。
...
以下为 J13g 历史范围：最终产品 `39cf9d47` 在 `ModOnboardingBehavior.cs:841–859` 的 `ProcessPendingBaseUrlValidationResult` 消费处再次核验取消版本，`:2486–2494` 取消时清除版本；`:142,158–159,173,2393–2404` 为三路 pending 结果发布标志建立跨线程可见性；前一片五个 UI/Onboarding owner 及原 Gauntlet/Campaign 适配责任见下方 f 范围。[代码地图](af-framework-code-map.json)当时绑定最终产品修订、**722** 锚点，两种核验模式均通过；仅作源码定位，不替代行为验收。原脚本 Debug/Release × 1.3/1.4 + Bootstrap 六构建、当前 Debug 1.4 DLL 回放、API/保存/Bridge/readiness 和相关回归的详细候选身份与证据见[主台账](../animusforge-refactoring-and-repository-reorganization-plan.md)。**`J13_OFFLINE_VERIFIED / J13g_OFFLINE_VERIFIED`** 仅限离线；实际游戏、旧档、provider、Gauntlet 焦点与帧性能仍 **NOT-RUN**，J14 当时未启动，也没有 Stage/部署/打包/push。

## 以下为 f 范围（历史）

# 当前范围：主体 J13f UI/Overlay/Onboarding 有限离线收口（2026-09-25）

产品/回放 `1e2bd80a`、`a2e9b8eb`、`26bc72fa`、`48bcc48a`：`src/modules/AF.Module.UI/WeeklyReportPopupSessionOwner.cs:5–71` 持有周报弹窗 dwell、百科暂停/恢复、Escape 与一次关闭；`src/modules/AF.Module.Onboarding/OnboardingSessionOwner.cs:5–65` 持有启动提示时机/幂等，`OnboardingDismissalOwner.cs:6–35` 持有意外关闭同阶段 150ms 恢复，`OnboardingOperationVersionOwner.cs:5–35` 持有三路异步取消代数，`OnboardingUiDispatchOwner.cs:7–50` 持有 VM 主线程有界回送/关闭失效。原 `DevWeeklyReportPopup.cs:40–65,79–106,221–260`、`AnimusForgeNativeConversationOverlay.cs:179–203,1629–1686` 和 `AnimusForgeApiOnboardingPopup.cs:43–64,157–213` 留 Gauntlet layer/焦点/暂停/Native 展示适配；打开失败均清理未发布候选。`AnimusForgeApiOnboardingVM.cs:815–848,853–987,1102–1130` 不再从 worker 直接改 VM，取消/关闭拒绝迟到 UI，短 Key 不明文显示，失败状态不回显原始响应。`ModOnboardingBehavior.cs:300–329,758–842,897–960,1817–1869,2346–2400,2572–2635,2886–2985` 留原 Campaign/保存键/MCM/网络请求/TW inquiry，取消源在 worker 启动前创建，结果由版本 owner 门禁；原 `ApplicationTickComposition.cs` 顺序和百科真实 root/datasource/输入拦截不改。当前 DLL owner 回放、Native admission 44/44、presentation 46/46、双版本 Debug 与 722 锚点地图见[主台账](../animusforge-refactoring-and-repository-reorganization-plan.md)；源码接线断言不是实机点击。**J13f `OFFLINE_VERIFIED`、J13 `ACTIVE`，下一包 g 最终离线门禁**。真实 Gauntlet/输入焦点/百科点击、Campaign、provider、旧档、帧性能 **NOT-RUN**；无 push、Stage、部署、打包或游戏/外仓写入。
...
# 历史范围：主体 J13 暂停，d1 有限离线收口，d2 尚未实施（2026-09-25）
...
`f54fbcf6` 将材料聚合的克隆、action 分类/事件键分桶和最终排序交给 `src/modules/AF.Module.Weekly/Materials/WeeklyMaterialAggregationOwner.cs:6,8`；真实自动预算消费者 `MyBehavior.cs:6301–6321` 与同步预览消费者 `:37220–37227` 经过原 host 薄入口 `:38550–38553` 共用 owner，live 名称/详情渲染留主线程。当前候选双版本六构建、Phase8 和新增分桶回放通过，440 锚点地图只证明定位；组内 O(M log M) 聚合与初始化 O(N) snapshot 尚无严格帧预算证明，J13a 全包仍 ACTIVE。详见[主台账](../animusforge-refactoring-and-repository-reorganization-plan.md#j13-plan-20260924)。
