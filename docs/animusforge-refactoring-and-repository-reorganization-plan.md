...
状态：**`J14_G0_BASELINE_VERIFIED / J14a_OFFLINE_VERIFIED / J14_ACTIVE`**。这只关闭[本轮 J14 计划](plans/j14-public-api-plan.md)的开工基线与 Scene 包，不表示 Courier/J14c、全仓 G0.7、实机或发布已完成。前置 `cbf874ac`、`40c0ccc5` 分别结算队列丢弃与 typed 后处理；本包产品/测试提交 **`1740b338`**。保留原未跟踪 `.dotnet-cli-home/`，未 push、Stage、Deploy、打包、写游戏/外仓/存档或改自动化。

- **唯一实际链路**：`AfDialogueClient.CaptureSceneContextTicket`/`SubmitScene` → `CoreDialogueClient` 同一 128-ID 指纹表 → `ShoutBehavior.ModuleSceneContext.cs:10–27` 主线程签发/一次 claim 并核原框选 source → `ShoutBehavior.ModuleSceneSubmission.cs:250–285` 原 `ProcessCapturedScenePlayerShoutAsync` → `ShoutBehavior.SceneConversationChains.cs:771–1454` 原 per-Hero 群组/接力/旁听/唯一后处理。`SceneGroupReceipt`（同文件 `:152–236`）等待本组有序发言的实际 speech TCS、typed 后处理与严格历史/AFEF 接受；退役立即结算 partial `UnknownAfterStart`，晚网络结果不能写进新场景。`ShoutBehavior.cs:17198–17220,26380–26434,26559–27086,36882–36950` 保留原游戏主线程/队列/历史适配，队列丢弃或未实际发声不伪造成功；`MyBehavior.DialogueHistoryCommit.cs:12–53` 旧七参签名不变，新增仅内部九参 Scene 定向玩家记忆接受。公共 `AfApi.cs:23` 的 `SceneSubmit=Available`，`AfDialogueClient.cs:15–89` 只暴露 detached 有序发言和不透明票据；Courier 与任意 Action/Memory/Extension 仍 `NotSupported`。公共 API 不替换 UI 默认入口。
- **频率与保留责任**：票据只在显式主线程捕获时签发，Scene owner 待领票据最多 128；提交沿每 client 128 ID 上限，群组回执只保存本次发言/依赖，不新增每帧全请求扫描。live Hero/Mission 捕获和回写在主线程，独立路由/知识检索在 worker，await 后重验 owner/generation/session/epoch/目标。API 每个发言的持久历史在主线程同步装配，**帧耗时未实机测量**；不以此离线结果豁免性能门。原 UI/原动作和故事规则仍由现存 host/领域 owner 持有，Scene 无第二套 parser、LLM 或游戏写入器。
- **当前候选离线证据**：`SceneGroupReceiptTests` 18（含等待中退役）；`SceneRequestLifetimeRegressionTests` 34；`ScenePostprocessParityTests/run_queue.py` 37；`SceneSpeechQueueOwnerTests` 6；`SceneConversationScopeTests` 5；`ChannelCutoverBoundaryTests` 132 + extraction 14；`NativeCompletionBoundaryTests` 186（含定向 Hero/非 Hero 记忆身份）；`NativeModuleSubmissionTests` 正常与内部 enum 重排各 55，外部 internal 访问预期 `CS0122`，精确旧 Native/新增 Scene hunk inverse 通过；`ModuleFrameworkApiTests` 142 与四份实际 DLL metadata 1252。所有 fixture/game/provider 层均不冒充实机。原一键脚本仅在用户授权的四个精确仓内产物目录复核无 reparse 后、不带 `-Stage/-Deploy` 跑 Debug/Release × 1.3/1.4 + Bootstrap 六构建，各 0 warning/0 error。四实现 SHA256：Debug 1.3 `22482E25AFBA54976E7CADC429CF8CBBDDDEBF74C756688937B6A02833408DBB`，Debug 1.4 `302DB8D054B5BAAEA040864E01AD4DB3E9B61D2F98FF376D975333DC79E4E11E`，Release 1.3 `C749BD8E3289B2540EBD64C08476360C95F9E6653AD9BEA089C61BB2680210F8`，Release 1.4 `F92C83ADE37F214885A62C6BF046862A45C70C25CAD72E363423D1E82806F447`。[代码地图](architecture/af-framework-code-map.json)绑定 `1740b338`、730 锚点 recorded/working-tree 通过，仅供定位。
- **非本包验证/下一动作**：旧 `MemorySummaryMainThreadBoundaryTests/run_commit_writers.py` 此次尝试因其既存 `TagSceneSessionHistoryLine` 抽取锚已不存在而在刷新基线阶段失败；不计入通过，也不是本包实际 Scene/NPC 记忆回执的替代。底层写入在本包 `NativeCompletionBoundaryTests` 仍是显式 stub；真实游戏记忆、provider、音频、旧 SAVE、两版本实机、帧耗时和独立子 MOD 游戏加载 **NOT-RUN**。下一包按计划做 J14b Courier 草稿/运输/到达/回信权威阶段，再做 J14c 全三渠道最终候选；不能把当前 Scene 收口称为整体 J14 完成。
...
- **真实责任/消费者**：`src/modules/AF.Module.Conversation/Channels/Scene/ScenePlayerShoutRequestOwner.cs:56–143` 的 `ScenePlayerShoutContext`/`CaptureContext` 只读记录 owner、Mission、Player、generation、session、epoch、输入序号和原框选，`TryClaimContext` 用 compare-exchange 一次领取；`ShoutBehavior.ModuleSceneContext.cs:11–105` 在主线程读取当前框选、复制候选列表/距离，并在领取前核原 UI source 与 Agent 实例，避免索引复用或同代静默改选。生产入口 `ShoutBehavior.cs:26188–26240,26385–26390` 仍由现有玩家喊话路径调用；本片令 `ProcessCurrentScenePlayerShout` 返回真实群组 Task、`ProcessCapturedScenePlayerShoutAsync` 在观察作用域外等待它，不再把启动 `Task.Run` 当完成。群组实际逻辑仍在 `ShoutBehavior.SceneConversationChains.cs:357–374,770+`，历史/标签/事实提交仍归既有 owner；新增内部上下文尚无 client 绑定或公开票据。
- **正反证据层级**：`tools/SceneRequestLifetimeRegressionTests/run.py` 提取当前 owner 与真实主线程 host，正常 **32/32**；`context-capture-advances` 和 `context-source-bypass` 变异编译成功并在具名行为断言失败。`--j14-completion` 针对前驱 `b4e093a1` 编译成功且在 `scene-group-must-not-complete-on-input-dispatch` 失败，当前 `b6c18a5c` 通过；它只证明等待群组 Task，不证明权威成功。Courier 测试从 `GenerationLifecycle.cs:488–570` 提取真实 `FinalizeCourierReplyGenerationOnMainThread`，**552/552、76 场景**；`preflight-implies-delivery` 变异编译成功且失败，证明预生成不代表 `DeliveryApplied` 或 `PostprocessConsumed`。独立 Native 公共消费者 **41** 检查通过，外部 internal 访问按预期 `CS0122`；Scene postprocess **71** fixture、speech queue owner **6/6** 通过。源码/fixture 与消费者证据均不冒充游戏实机。
- **兼容/构建**：本轮用户只授权精确递归重建 `bin/Debug/single_module_artifacts`、`bin/Release/single_module_artifacts`、`obj/single_module/Debug`、`obj/single_module/Release`；执行前复核四个绝对路径、内容和父/子路径无 reparse。原 `build_single_module.ps1` 不带 `-Stage/-Deploy` 完成 Debug/Release × Bannerlord `v1.3.15.110062` / `v1.4.7.117484` + Bootstrap 六构建，均 **0 warning/0 error**。四实现 SHA256：Debug 1.3 `4F9A74A8D761D4771397151DD2EE26C20571A5385FFB849C242892980C43CDA9`，Debug 1.4 `1695874AC4D9B4785B19DCD62749658B4553C465CBFA9F9EBED25C8DC98C5853`，Release 1.3 `923A20AD47193CC7EE3372DF5A1B14558B34B06016414D6456A4839A9BA4BDFB`，Release 1.4 `57C6966EFEEE284E23F1DCAF165A99E96D1CDFC88B5925AEE709A6DB909A3756`。[代码地图](architecture/af-framework-code-map.json)绑定产品 `b6c18a5c`、**726** 锚点 recorded/working-tree 均通过，只供定位；`NativeModuleSubmissionTests/source_boundary.py` 通过。
- **未覆盖与下一动作**：Scene 群组仍有吞异常/早退、worker 上的 live Hero 读取、接力/相关旁听及动作/历史/AFEF 权威完成未统一回执；等待 Task 返回不能推断成功，也没有本次可见发言的不可变快照。下一片先沿 `HandleGroupResponsePerHeroIndependent` 的成功、失效、早退和异常路径建立 typed 群组回执，并隔离后台 live 对象，再做 client 绑定有界票据/渠道 fingerprint 与公共接线；随后按计划做 Courier 草稿竞争/真实运输阶段，最后 J14c 离线矩阵。实机 1.3/1.4、旧档、provider、音频、帧性能、外部 Scene/Courier 消费者均 **NOT-RUN**。未 push、Stage、部署、打包、写游戏/外仓/存档、改自动化或进入 J15；`.dotnet-cli-home/` 保留。

## J14 三渠道公共 API 开工意图（历史）

状态：**`J14_ACTIVE / G0_ACTIVE`**；实际 Git 根 `E:/AnimusForge-refactor-continuation-20260831`、分支 `codex/af-main-refactor-continuation-20260831`、开工 HEAD `16c180b38dd058b34f80d28cf01ca2a762800b2c`，已跟踪/暂存无改动，原未跟踪 `.dotnet-cli-home/` 保留。本次用户明确授权执行[已保存计划](plans/j14-public-api-plan.md)，覆盖历史 J13 交接中“J14 需另行授权”的边界；不授权外部发布或清理。J13g 最终产品 `39cf9d47` 的有限离线结果是基线，不充当 J14 验收。

- **冻结目标与真实 owner**：Scene 只提交 AF 当前有效框选/主对象，Courier 只提交原流程已完成选择的待发送草稿；主线程签发 client 绑定的不透明票据，查询不推进 Scene input sequence、不派队或触发 LLM。`AfDialogueClient`/`CoreDialogueClient` 保留 Native ABI 与每 client 128 ID 有界去重；Scene `ScenePlayerShoutRequestOwner` 与 `ShoutBehavior.ProcessCapturedScenePlayerShoutAsync`/群组链仍是真实准入和完成 owner；Courier `CourierDeliveryBehavior.SessionCreation`/`GenerationLifecycle`/`CommitDispatch`/`DeliveryLifetime` 是派出、到达、提交和返回的唯一 owner。不将 `Task` 正常返回、预生成、`DeliveryApplied`、`PostprocessConsumed` 或 `Stage=Completed` 当全链成功。
- **消费者与状态**：独立消费者从 `AfApi.CreateDialogueClient` 进入，UI 仍使用现有 Scene/Courier 入口并共享准入。票据/关联仅 process-local，不增加保存键；排队取消无效果，claim 后为 TooLate/未知效果，owner 完成回执优先。任务频率为显式签发/提交与原阶段事件，不引入每帧全 client 扫描；完成后释放活对象关联。公共 DTO 只保留 detached 数据。`ActionExecute`/`MemoryWrite`/`ExtensionRegister` 不开放，Native 既有签名/枚举值不变。
- **有限退出门**：G0 跑现有 Native 外部消费者并增加 Scene 提前完成、Courier 预生成假完成的真实行为红例；J14a/b 各自完成票据→唯一真实 owner→必要动作/历史/事实→终态的正反回放和双版本兼容，之后才将相应能力改 Available；J14c 核旧 ABI、四实现、最终离线矩阵并更新指南/范围图/代码地图/HANDOFF。旧档、实机、provider、音频、帧性能继续 `NOT-RUN`，不能报发布 READY。构建脚本的递归清理须在本轮精确路径/内容/reparse 核对和授权后执行；未授权不继承旧 J13 许可。不 push、Stage、部署、打包、写游戏/外仓/存档、装全局工具、改自动化或进入 J15。
...
状态：**`J13e3_OFFLINE_VERIFIED / J13_ACTIVE`**，下一包 e4 Settlement/Inspection，随后 e5 Exercise；此结论仅覆盖离线契约/合成行为，不是实机会面验收。意图 `8229066f`；旧测试本机 SDK 参数 `6de7fd53`；生产/行为 `0b0b32c0`（选中目标）、`dc2bbe03`（释放和未授权返回）、`86c11889`（会话目标参数优先级）；聚合接线 `73bef755`。一基坐标和未覆盖责任见[范围图](architecture/af-framework-code-scope.md)、[代码地图](architecture/af-framework-code-map.json)，源码字符串契约不单独证明行为。

- **owner 与真实消费者**：`src/modules/AF.Module.Encounter/EncounterTargetOwner.cs:7–28` 保留合法已选军团成员并仅在失效时回退当前遭遇首领；`LordEncounterBehavior.cs:33–39,4871,4898–4945,5314–5329` 持有 owner、提供当前 party/army eligibility，菜单重复检查不覆盖合法选择。`EncounterConversationTargetOwner.cs:6–27` 实际决定显式参数→instance→遭遇首领，`EncounterConversationTargetResolver.cs:17–37` 仍做 Bannerlord/反射提取、合法 lord 与显式 prisoner 放行；三个原会话 patch 消费该 resolver 并 `SetTarget`，不新增注册。`EncounterReleaseOwner.cs:7–86` 独占授权/延迟请求和失效/120s/10s/0.25s 状态；`LordEncounterBehavior.cs:83–110,301–336,7280–7376` 保留原 TW request、Mission 结束及 safe-passage/原生副作用。`EncounterPendingReturnOwner.cs:7–36` 锁定 encounter/party/save generation；原 host `:138–140,359–376,694,3728–3814` 只在同一 MapState/身份执行未授权离场返回。原 Campaign/tick/三个 Harmony patch 装配保持一份；保存身份未改。
- **受控证据**：`EncounterLifecycleBoundaryTests/run.py --dotnet local/dotnet/8.0.425/dotnet.exe` 的真实方法提取+四个源码链接 owner **53/53**（原基线 35）涵盖已选军团成员、重复检查/换遭遇、会话参数顺序、release 换 encounter/party/Mission/save/过期/重入和未授权返回失效。Debug 1.4 当前生产 DLL SHA256 `8545757A9A8880F1E5D40E74E7F0727D967DD86C6675442E571232293B803ACD` 的完整 Phase8 含四个 owner 反射行为回放；`J13E3DomainOwnerContractReplay` 另核对生产消费者/原三个 patch/装配，明确仅 source wiring。原构建脚本四个仓内递归重建目标先按绝对路径和 nested reparse 复核，不带 `-Stage/-Deploy` 的 Debug/Release × Bannerlord `v1.3.15.110062`/`v1.4.7.117484` + Bootstrap 六构建均 0 警告/错误；V1 **119**、四 DLL metadata **1100**、PersistenceIdentity **142/36**、迁移 fixture **10**、source inventory **7**、代码地图 **663** 锚点 recorded/working-tree 通过。
- **频率/边界**：owner 仅在原会话入口/菜单条件/释放 tick 的已有调用点运行；会话参数扫描只按原 `args` 长度，静态 delegate 避免每次分配；其余身份检查 O(1)，没有新增全局扫描、反射、锁或轮询。原始 TW 实例/事件顺序、Harmony 命中、实机两版本会面、真实旧档、native 退出/safe-passage、帧性能均 **NOT-RUN**；六构建只证明双版本编译。未 push、Stage、部署、打包、写游戏/外仓或改自动化；`.dotnet-cli-home/` 保留。

## 主体 J13e4 Settlement/Inspection 开工意图（2026-09-25）

状态：`J13e4_ACTIVE / J13_ACTIVE`，基线 `45b843f9`。按[原计划](plans/j13-domain-owners-plan.md)仅推进入场选择、临时队伍/Agent 状态和离场清理，随后 e5，不提前 J14。已核对 `SettlementEntryTroopSelectionBehavior.cs` 原 Campaign 注册/保存/入场 patch、pending mission entry 的 30s 过期与 MissionStarted 消费、selected follower mission/index 状态；`TroopInspectionBehavior.cs` 原队伍选择、0.35s 延迟开 Mission、临时 roster/holding party/map event/PlayerEncounter、取消/失败/离场 cleanup。先给 SETS pending 入场和 follower 状态真实 owner/失效回放，再给 Inspection 暂态/清理 owner 和生产回放；保留 TW 操作、原事件/patch/保存键。GCCZ 只核对原薄接缝，不改业务或外仓。真实游戏入场/死亡/旧档/帧性能不能由离线模拟代替；`.dotnet-cli-home/` 保留，无 push、Stage、部署、打包、游戏/外仓写入或自动化。

## 主体 J13e4 Settlement/Inspection 有限离线收口（2026-09-25）

状态：**`J13e4_OFFLINE_VERIFIED / J13_ACTIVE`**，只表示本包的离线 owner、消费者和兼容门禁通过；下一包 e5 Exercise，不提前 J14。意图 `8d611a7f`，生产/当前 DLL 行为切片 `a946301a`（入场 ticket）、`cf162dd5`（Mission/Agent 身份）、`35daa7a6`（Inspection session/清理），聚合契约 `ef6a83e1`。一基坐标、保留与未覆盖责任见[范围图](architecture/af-framework-code-scope.md)、[代码地图](architecture/af-framework-code-map.json)；源码接线契约不单独证明游戏行为。

- **真实 owner 与生产路径**：`src/modules/AF.Module.Settlement/SettlementMissionEntryOwner.cs:6–62` 持有一次性 pending 入场，负责 GCCZ 村庄取消、同地点 MissionStarted 消费、错地点拒绝与 30 秒超时；`SettlementEntryTroopSelectionBehavior.cs:72–79,236–243,1460–1494,1676–1719` 仍完成原 profile roster/TW Mission 适配。`SettlementFollowerMissionOwner.cs:7–60` 以 Mission 和实际 Agent 引用绑定选中随行，index 仅用于 O(1) 查找，索引复用不冒充旧 Agent；原 host `:87–100,270–295,952–1004,3155–3214,3216–3240` 仍负责生成、阵型/伤害、移除和 Mission 结束，死亡后剔除对应引用。`TroopInspectionSessionOwner.cs:4–88` 持有选择/临时 runtime、0.35 秒队列、开场/清理和载入失效状态；`TroopInspectionBehavior.cs:31,112–128,175–223,226–290,303–410,532–568,1740–1752` 仍执行原 PartyScreen、roster/holding dummy party、MapEvent/PlayerEncounter、角色/Hero 恢复与原 Mission 清理回调。新游戏/读档由原 SETS Campaign 事件调用 `ResetForCampaignTransition`，仅丢弃旧 Campaign 静态引用，不把旧游戏对象恢复进新游戏。原保存键、三个地点入场 patch、Inspection 五 patch 和 Terminal tick/打开入口保留；GCCZ 的准备/取消/城堡场景薄接缝仅核对未改业务与外仓。
- **受控证据**：当前 Debug 1.4 生产 DLL SHA256 `877CE07F95D20F7DD82FEFAFA89D873CB3C831E0CC9240A5A5B5B39EAEA2BE39` 完整 Phase8 含三个实际 owner 的 one-shot/错地点/GCCZ 取消/期限、Agent 索引复用/死亡/Mission 切换、选择取消/队列截止/重复清理/载入失效回放；`J13E4DomainOwnerContractReplay` 另核对原 Campaign/Harmony/保存/Terminal/GCCZ 消费者和原清理链，明示仅源码接线。每个产品切片均先复核原脚本四个仓内递归重建目标与 nested reparse，不带 `-Stage/-Deploy` 完成 Debug/Release × Bannerlord `v1.3.15.110062`/`v1.4.7.117484` + Bootstrap 六构建，均 0 警告/错误；最终四实现 SHA256 分别为 Debug 1.3 `5B6C8DEB7FEC658BB69E76C6D770D01A25F0B434A5A9BA741D95792ED1870157`、Debug 1.4 如上、Release 1.3 `B0BA87DFBBAC7A24B152395B45EF4C8F69C65F705798E786BC070F6CBEB542F6`、Release 1.4 `46ADF251BC5756EF2A458E17DA97010C20076D23A5E2E02004162506DD0729A6`。V1 **119**、四 DLL metadata **1112**、PersistenceIdentity **142/36**、迁移 fixture **10**、source inventory **7**、代码地图 **680** 锚点 recorded/working-tree 通过。
- **频率/未验**：入场过期仍只在原 Campaign tick 检查单个 ticket；随行表只在生成/判定/AgentRemoved/结束时 O(1) 使用，无新每帧全场扫描；Inspection 复用原按需 Terminal engine tick，未增加反射/锁/轮询。真实 1.3/1.4 场景入场、PartyScreen 取消、角色/队伍/MapEvent 副作用与原生清理顺序、GCCZ 实机交接、死亡/中止、旧档加载及帧性能均 **NOT-RUN**；离线回放不能代替实机验收。未 push、Stage、部署、打包、写游戏/外仓或改自动化；`.dotnet-cli-home/` 原样保留。

## 主体 J13e5 Exercise 开工意图（2026-09-25）

状态：`J13e5_ACTIVE / J13_ACTIVE`，基线 `6fad519b`。按[原计划](plans/j13-domain-owners-plan.md)只推进军事演习初始化、临时 XP-only 结算、结束恢复；不提前 J14。原 `MilitaryExerciseBehavior.cs` 持有两轮队伍选择/0.2s 与 0.35s 延迟、MapEvent/双 dummy party、XP-only 回执和全套 roster/role/Hero 回收；`AnimusForgeTerminalBehavior.cs:98–102,471` 为现有打开/tick 消费者，`StartupPatchComposition.cs:461` 为原 patch 装配。已复核 `GetBattleRewards` 的 1.3 float/out 与 1.4 `ExplainedNumber`/out 双签名，以及 MapEvent/PlayerEncounter 和死亡率前缀必须仅命中当前演习，不得误吞原版战斗。先迁选择/排队及结算授权状态到真正 Exercise owner，保持 TW 操作和双版本补丁签名，再做当前 DLL 行为回放、聚合契约、双版本构建与有限离线收口。真实游戏结果、旧档、经济/伤害/帧性能仍须实机；`.dotnet-cli-home/` 保留，无 push、Stage、部署、打包、游戏/外仓写入或自动化。

## 主体 J13e5 Exercise 有限离线收口（2026-09-25）

状态：**`J13e5_OFFLINE_VERIFIED / J13_ACTIVE`**，只限本包的代码、当前 DLL 回放、聚合接线和双版本构建；J13f/g 尚未开始，J14 不开始。意图 `d7a73967`；各验证切片 `ba645b29`（精确 MapEvent/真实战斗排除）、`def2b082`（两轮选择/延迟 Mission）、`3d00d416`（临时 XP-only 回执/一次结算）、`80e21be1`（重入/迟到选队回调与聚合契约）、`8654427a`（孤立 MapEvent 一次 XP 回执）。一基路径/符号/覆盖与保留责任见[代码范围图](architecture/af-framework-code-scope.md)和[代码地图](architecture/af-framework-code-map.json)，地图仅用于导航，不代替运行行为。

- **真实 owner 和生产消费**：`ExerciseMapEventIdentityOwner.cs:6–17` 只承认当前 MapEvent 或包含本次两个实际 dummy party 的事件；原 `MilitaryExerciseBehavior.cs:1358–1419` 的孤立恢复还要求专用 `MilitaryExerciseDummyPartyComponent` 与 ID 前缀，防止真实玩家战斗被演习 XP/奖励前缀误吞。`MilitaryExerciseSessionOwner.cs:6–99` 持有选择、0.2/0.35 秒 ticket、活动 runtime、打开/取消/释放；原宿主 `:911–1010,1012–1068,1784–1979` 仍开 PartyScreen/战斗 Mission，回调用捕获的选择引用拒绝迟到取消，已分队待开战时拒绝终端重入。`ExerciseSettlementOwner.cs:7–80` 为每场运行演习持有 XP 尝试/成功、原版 patch 命中、renown 跳过、结算和撤退兵恢复一次性状态；孤立事件使用 `ConditionalWeakTable` 按 MapEvent 身份收据，不强留历史事件。原宿主 `:1174–1354,2838–2900,3096–3140` 继续执行 TaleWorlds 的 XP、roster、角色、Hero、MapEvent/PlayerEncounter 和双 dummy party 实际恢复；不改保存键/类型、默认战斗结果、原构建脚本或公开 API。对某一侧 XP 已提交而另一侧失败的**不确定**结果不重试，避免重复发放；可能少发 XP 的实际副作用须实机观察。
- **补丁/消费者核对**：原 Terminal `AnimusForgeTerminalBehavior.cs:98–102,469–472` 按需 tick/打开、`StartupPatchComposition.cs:461` 唯一注册点保留。`MilitaryExerciseBehavior.cs:653–690` 的 `SandboxAgentDecideKilledOrUnconsciousModel.GetAgentStateProbability` Postfix 仅在当前 Mission 有演习逻辑时调整人类死亡率；MCM 没有“关闭整个演习”开关，只有死亡率及玩家/同伴可战死设置，不将关掉可战死误说成退出整套演习。`MapEvent.CalculateAndCommitMapEventResults` 前缀 `:696–713` 和 `PlayerEncounter.DoApplyMapEventResults/DoWait/DoPlayerVictory/DoPlayerDefeat/DoEnd` `:715–798` 仍按演习事件/孤立专用 party 门禁，真实原版战斗返回原流程；`GetBattleRewards` `:799–864` 保留 1.3 九参数 float/out/ref 与 1.4 五参数 `ExplainedNumber`/out，两版本 Debug/Release 四个 DLL 元数据均核对；`ApplyRenownAndInfluenceChanges` `:867–887` 只在 1.3 编译注册。原 no-retreat 五 patch、Mission `OnEndMission` 早期 XP 与 side-defeat 清理、双 dummy party/roster/角色/Hero/Encounter 归还链不重排，聚合源码契约逐项检查，但不以字符串断言代替真实运行。
- **离线验证**：每次运行原脚本前复核 `bin/{Debug,Release}/single_module_artifacts` 与 `obj/single_module/{Debug,Release}` 四个仓内递归重建目标及 nested reparse；不带 `-Stage/-Deploy` 的 Debug/Release × Bannerlord `v1.3.15.110062` / `v1.4.7.117484` + Bootstrap 六构建均 0 warning/error。最终四实现 SHA256：Debug 1.3 `AE227E48B6654C8EAAD5212307DE21DBE777E4DB845BA222AAA27B9AD101FE9D`、Debug 1.4 `2F9D0E976A18CA4C28FDA20B4C6C728886EE770AF9769EBC825B5D8BEBFD02E8`、Release 1.3 `F797745E242D96585B4F09BFD753A5B804FD20144A3497136CA6A9115B5A6634`、Release 1.4 `B3D716B5740C82374AE613056FE3ED6FCF35A6C803C1F5B98C72D1CD10C023A6`。当前 Debug 1.4 完整 Phase8 直接加载此 SHA 候选，包含身份正/真实战斗反例、两轮截止/一次消费/迟到身份、XP 成功/部分失败/早期结束/孤立事件/重复清理回放；`J13E5DomainOwnerContractReplay` 另核对 Terminal/Harmony/Mission/TW 清理与四 DLL 奖励参数，明示源码接线。V1 **119**、四 DLL metadata **1128**、PersistenceIdentity **142/36**、迁移 fixture **10**、source inventory **7**；代码地图 **708** 锚点在记录提交与工作树两模式通过。
- **性能和未验**：owner 的状态/ticket/身份检查均 O(1)，仅复用原 Terminal 按需 tick；孤立收据仅在相关 MapEvent patch 时创建，不新增全局扫描、锁、反射或空转轮询。原 MapEvent party 侧枚举、XP 反射提交与 roster 恢复只在原事件/结算路径。真实 1.3/1.4 Mission/Harmony 时序、MCM 设置在游戏中的实际伤害/奖励、战斗经济与伤亡、异常时 TW 资源回收、旧档、帧性能均 **NOT-RUN**；现有清理 `finally` 会标记完成，即使 TW 侧清理异常，离线证据不能保证实机总能恢复。未 push、Stage、部署、打包、写游戏/外仓或改自动化；`.dotnet-cli-home/` 保留。

## 主体 J13f UI/Overlay/Onboarding 开工意图（2026-09-25）

状态：`J13f_ACTIVE / J13_ACTIVE`，基线 `be5571cd`。按[原计划](plans/j13-domain-owners-plan.md)只推进 UI 薄适配、原 Native Overlay 生命周期和 Onboarding；完成 f 后才做 g 最终离线收口，不提前 J14。现有 `ApplicationTickComposition.cs:47–69,88–109` 是 popup/Overlay/百科/Onboarding 的原顺序入口；`EncyclopediaHeroPersonaPatch.cs:59–180,816–1008` 保留真实 Gauntlet root、datasource 和事件门禁；`AnimusForgeNativeConversationOverlay.cs:101–201,277–305,312–566,1628–1667` 保留 `ShoutBehavior.NativeAdmission.cs:217–257` 的唯一 Native 提交/展示接线。`ModOnboardingBehavior.cs:228–365,783–875,1124–1282,3135–3215,3338–3373` 仍为 Campaign/保存/MCM/网络/弹窗混合宿主，计划把启动、阶段、取消版本与失效决策归有界 Onboarding owner，保留 `_AnimusForge_setup_done_v1`、默认设置、焦点与原入口。先写关闭/重复打开、换 screen/owner、迟到回调、焦点/暂停释放及启动幂等反例，再做最小产品改动和当前 DLL 回放；源码/XML 检查只作接线证据。Gauntlet 实机点击/渲染、真实 provider、旧档、帧性能均 `NOT-RUN`。用户仅授权原脚本在最终构建前递归重建仓内四个已核实的 Debug/Release `bin/single_module_artifacts` 与 `obj/single_module` 目录；每次仍重查解析路径和 nested reparse，不 Stage/Deploy/打包/push、写游戏/外仓、改自动化或清理 `.dotnet-cli-home/`。

## 主体 J13f UI/Overlay/Onboarding 有限离线收口（2026-09-25）

状态：**`J13f_OFFLINE_VERIFIED / J13_ACTIVE`**，只限此包当前代码/生产 DLL 回放/双版本 Debug 和接线；J13g 最终 Release/API/保存/Bridge 门禁尚未开始，J14 不启动。意图 `c79bdeac`，产品/测试切片 `1e2bd80a`（Weekly 弹窗会话）、`a2e9b8eb`（启动提示）、`26bc72fa`（Gauntlet 部分失败清理、Onboarding 意外关闭/VM 异步回送）、`48bcc48a`（三路 API/URL/模型拉取取消版本）；Native presentation 测试入口 SDK 修复 `61f8720a`。一基 owner、宿主、覆盖/保留责任见[范围图](architecture/af-framework-code-scope.md)与[代码地图](architecture/af-framework-code-map.json)，地图仅导航，不是玩法验收。

- **真实责任和消费者**：`src/modules/AF.Module.UI/WeeklyReportPopupSessionOwner.cs:5–71` 承接最短停留、百科暂停时长扣除、350ms 恢复后 Escape 防误关和一次性关闭；`DevWeeklyReportPopup.cs:40–65,79–106,157–260` 保留 Gauntlet layer、焦点、pause 与百科链接。`src/modules/AF.Module.Onboarding/OnboardingSessionOwner.cs:5–65` 承接启动提示的 1/2/3 秒与幂等，`OnboardingDismissalOwner.cs:6–35` 承接同阶段意外关闭 150ms 恢复一次授权，`OnboardingOperationVersionOwner.cs:5–35` 承接 API/Base URL/模型拉取的线程安全代数与取消失效，`OnboardingUiDispatchOwner.cs:7–50` 承接 VM 回调主线程限量执行、测试取消及关闭退休；原 `ModOnboardingBehavior.cs` 保留 CampaignBehavior 身份、`_AnimusForge_setup_done_v1`、MCM/网络/TW inquiry 与原保存/退出语义，`AnimusForgeApiOnboardingVM.cs` 只把模型/握手结果回送主线程，单帧最多消费 32 项。三个原 UI 宿主现在在 Open 中途失败时回收未发布候选，Onboarding 释放 input mask/焦点/pause，Native 回收 input、原生 answer area 和 layer；原 `ShoutBehavior.NativeAdmission`、Native 默认入口、`ApplicationTickComposition` 顺序与百科真实 root/datasource/文本输入快捷键拦截均不变。短 YJ Key 不再原样展示，VM 失败状态改用固定提示/分类提示而非原始异常或响应。
- **验证层级**：原脚本在每次执行前核查已授权仓内 Debug 的 `bin/single_module_artifacts` 与 `obj/single_module` 解析路径和 nested reparse，未带 `-Stage/-Deploy` 完成 Bannerlord `v1.3.15.110062`/`v1.4.7.117484` + Bootstrap 3 个 Debug 构建，均 0 warning/error。当前 Debug 1.4 生产 DLL SHA256 `E07886118A440F3B462EE7C6A0182213B5C00BD42391AF76435E52EDD51156A5` 完整 Phase8 直接加载此候选，包含 Weekly dwell/暂停/重复关闭、Onboarding 启动时机/意外关闭/三路取消代数/VM 队列失效等当前 DLL 行为回放；`J13FUiHostLifecycleContractReplay` 仅核对真实消费者源码/失败回收和取消源先于 worker 建立。Native admission 提取回放 **44/44**、presentation **46/46**（下游 stub，非实机），source inventory **7**；代码地图 **722** 锚点 recorded/working-tree 通过，仅定位。没有真实 Key、响应或 Token 写入测试与交接。
- **性能/残留风险**：新 owner 的 Tick 决策 O(1)，VM 每帧限 32 个已完成 UI 回调，无新增每帧全量扫描/反射/空转；网络回调仍依原请求频率，关闭后丢弃 UI 回送。原 Onboarding 其他错误详情仍通过已有 `NonBlockingErrorReport.RedactConfiguredApiKeys` 路径处理，不把本包 VM 的泛化提示误称全渠道脱敏验收。Gauntlet 实际点击/渲染/焦点、百科输入键处理、Campaign 开始与退出/旧档、真实 provider、TW 异步/取消竞态和帧性能均 **NOT-RUN**；源码契约与 stub 不可替代这些验收。未 push、Stage、部署、打包、写游戏/外仓或改自动化；`.dotnet-cli-home/` 未跟踪目录保留。

## 主体 J13g 最终离线收口（2026-09-25）

状态：**`J13_OFFLINE_VERIFIED / J13g_OFFLINE_VERIFIED`**，只表示[原计划](plans/j13-domain-owners-plan.md) J13a–f 已完成其限定离线 owner/接线回放，g 的最终候选六构建/API/保存/Bridge/readiness/代码地图门禁完成；不是 J14、全项目发布、真实游戏/旧档/provider 或性能验收。f 本包产品基线 `48bcc48a`；g 的证据清单修正 `894acdfc` 只修 Profile 的嵌套调用提取及 168 绑定坐标/两处经核实的 ref 名、Phase8 入口候选，**不改存档键/类型/来源、owner assignment、coverage 状态或玩法**。最终产品补丁 `39cf9d47` 在 Base URL 异步 worker 已发布结果后、玩家取消且主线程下一 Tick 尚未消费的缝隙再次核验版本；原先可把迟到成功当本次流程消费，现取消后拒绝。三路 pending 发布标志用 release/acquire 语义可见；源码契约先红后绿。此补丁及之前五个 J13f owner 的实际消费者见[范围图](architecture/af-framework-code-scope.md)和[代码地图](architecture/af-framework-code-map.json)。

- **最终六构建与候选身份**：每次原 `一键编译覆盖推送/build_single_module.ps1` 前核实用户精确授权的 Debug/Release 两组 `bin/<Configuration>/single_module_artifacts`、`obj/single_module/<Configuration>` 解析后仍在仓内且无 root/nested reparse；按已核实的配置及依赖路径参数执行，不传 `-Stage/-Deploy`。本机固定 SDK `local/dotnet/8.0.425`，1.3 引用 `_deps_auto` = `v1.3.15.110062`、1.4 引用 `local/bannerlord-refs/1.4.7.117484` = `v1.4.7.117484`，游戏与私有 runtime 仅作只读依赖输入。最终 Debug/Release × 1.3/1.4 + Bootstrap 六项均 **0 warning / 0 error**。四实现 DLL SHA256：Debug 1.3 `57BD5ED60914FEA3B43270E195AC843B29FB0E56927AB137CE29F41D7AB0A36A`，Debug 1.4 `03D0FACFC1199B7BFA68B8BDEEABCFF8B69E5B9A97AE6BB279050C33509F0E55`，Release 1.3 `8CDD642530179C499C1BCADA7445F1072B481FD73073535D55E355869039F9C2`，Release 1.4 `D1CC89ADB12E1653EDBE1A6CAA5F52D69478882AFB93830374C659B6C140E65A`；Debug/Release Bootstrap SHA256 `A1E8895DA59B69148F574454B9AC77AFC992F77CE8B1C26FFE6FD69083791E5E` / `3D6EB8F9FF5BD0CC2AF198ED895E521253EE77616E39A7DA7244C1C771441D76`。`build.json` 的 SHA 与实际文件对齐。
