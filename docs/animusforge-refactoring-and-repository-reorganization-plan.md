...
Courier 正常回归外，prompt-run 四个、detached postprocess 八个、commit outcome 四个变异均编译运行并命中具名断言；generation lifecycle 继续由 liveness `ignore-run` 变异命中“旧准备不得完成新 Start”具名断言。既有 whole-file 逆变换及旧 commit runner 的已登记阻塞未刷新/削弱来冒充 PASS。`c9ea2e88` 后 Debug 1.3/1.4/Bootstrap 均 0 warning/0 error；348 锚点地图绑定该产品提交并在 recorded/working-tree 两模式通过。下一包 J10b5 处理 `ProcessSession`、transport/arrival/inbound letter UI/route/retry。回滚按 `c9ea2e88` → `1882ea1c` → `5cb7dc6e` → `bdf58283` 定向 revert；Scene 回滚链保持，不 reset/rebase。
