...
> **J01c/J01d STOPPED（迁前来源门禁）：** 只读调用 `tools/package_policy_system_source_overlay.py` 的 `build_file_set()` 在 `:127→:43` 退出 `1`：`FileNotFoundError: AnimusForge/CustomPrompts/CustomPolicyEvaluatorPrompt.json`（清单 `:109`）。该路径不存在且未被 Git 跟踪；不伪造 overlay 集合对照、不补资产、不扩大本轮白名单。两原文件 hash 仍为 J3:90、目标不存在；未移动源码/消费者，未运行迁后测试或 Stage。`artifacts/workspace-j01-llm-protocol/after/` 已建空目录，失败产物保留；等待来源/门禁处置决定。
