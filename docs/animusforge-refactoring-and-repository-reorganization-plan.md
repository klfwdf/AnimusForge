...
- **当前候选离线证据**：`SceneGroupReceiptTests` 18（含等待中退役）；`SceneRequestLifetimeRegressionTests` 34；`ScenePostprocessParityTests/run_queue.py` 37；`SceneSpeechQueueOwnerTests` 6；`SceneConversationScopeTests` 5；`ChannelCutoverBoundaryTests` 133 + extraction 14；`NativeCompletionBoundaryTests` 186（含定向 Hero/非 Hero 记忆身份）；`NativeModuleSubmissionTests` 正常与内部 enum 重排各 55，外部 internal 访问预期 `CS0122`，精确旧 Native/新增 Scene hunk inverse 通过；`ModuleFrameworkApiTests` 142 与四份实际 DLL metadata 1252。所有 fixture/game/provider 层均不冒充实机。原一键脚本仅在用户授权的四个精确仓内产物目录复核无 reparse 后、不带 `-Stage/-Deploy` 跑 Debug/Release × 1.3/1.4 + Bootstrap 六构建，各 0 warning/0 error。四实现 SHA256：Debug 1.3 `9E58BB9AD9D7EBE5890C7BA9E62ED4072F2A38420912B4F69EDBFC1855B34D2C`，Debug 1.4 `E0E451CADE5CB3E9EB9A1CE46DBC0039EA477578D5F1C3A67287C4DE1A89F0CE`，Release 1.3 `8563D85C9F512DD7D68C5E9C855D6DF7531147836EA51298D5EFFA427BD57EC0`，Release 1.4 `27DD3952E1164BAC8E1F2F9E0BB603C835BC4D60460201D27A7FED32BAE6BC27`。[代码地图](architecture/af-framework-code-map.json)绑定 `79fa7c48`、730 锚点 recorded/working-tree 通过，仅供定位。
