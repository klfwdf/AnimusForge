...
- Validation owner (`LOCAL-8-A`): `tools/PhaseEightReadiness` keeps the existing8-ID design catalog/Bridge/Composition contracts, and additionally source-binds `docs/phase8/full-domain-readiness-catalog.json` plus `cleanup-candidates.json`. The20 entries are acceptance responsibility buckets, not deployed modules. Evidence must declare domain IDs and all related maintainers; each domain and all16 full-domain Bridges require OFFLINE/LIVE/SAVE coverage, while release/package remains separate. Cleanup replacement and rollback evidence bind exact candidate IDs; HEAD is not a rollback point. The tool still never authorizes cutover/deletion/deployment/push/publish and never upgrades fixture/Stage evidence into live proof.
