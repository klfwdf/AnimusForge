...
- Remaining joint owner work: MyBehavior still needs live AFEF/old-save acceptance. Courier inbound remains the one confirmed P1 outside this owner: memory recovery cannot complete its suppressed `afterCommit`, so Courier needs a separate persistent/idempotent session-completion receipt. Structured unknown receipts do not compensate gameplay effects; default cutover and destructive cleanup remain blocked.
