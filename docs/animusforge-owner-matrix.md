...
- Remaining joint owner work: MyBehavior still needs live AFEF/old-save acceptance, including weekly/notoriety auxiliary effects that H deliberately excludes from its core repair claim. I closes the known Courier `afterCommit` session P1 for newly created receipts, but H→I intermediate saves without a Courier receipt cannot recover the old visible reply. Structured unknown receipts do not compensate gameplay effects; default cutover and destructive cleanup remain blocked.
