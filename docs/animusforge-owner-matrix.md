...
- Remaining joint owner work: MyBehavior still needs live AFEF/old-save acceptance. Weekly has no outcome-aware exact-intent owner: the legacy candidate is created before action terminal state and lacks request/turn identity, while detached currently does not create the candidate at all. Notoriety lacks a durable per-line/session receipt and cannot be recovered safely. I closes the known Courier `afterCommit` session P1 for newly created receipts, but H→I intermediate saves without a Courier receipt cannot recover the old visible reply. Structured unknown receipts do not compensate gameplay effects; default cutover and destructive cleanup remain blocked.
