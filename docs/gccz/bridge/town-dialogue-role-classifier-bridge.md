...
- The AF adapter supplies live Bannerlord facts only. Role priority is prisoner, companion, accompanying noble, settlement notable, ordinary soldier, then ordinary civilian.
- Missing, legacy `Unknown`, and invalid role values normalize to `OrdinaryCivilian`, the lowest-authority ordinary scene role. The fallback cannot execute soldier orders or inherit named-Hero memory.
