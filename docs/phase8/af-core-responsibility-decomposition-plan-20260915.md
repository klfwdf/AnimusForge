...
73774a94完成C1请求生命周期基础修复/提取，main普通30项保持、5旧红/新51项通过，未放行B1或整体SDK。[本计划的main验收附表](af-core-main-closeout-matrix-20260915.md) / [当前HANDOFF](../handoffs/2026-09-15-main-closeout-lifetime-handoff.md)。其余生命周期/记忆/渠道职责及实机门槛保留。

## 以下为已有计划与历史进度

# 当前进度：E1 只读投影隔离与旧对照链收口（2026-09-15）

f07cb2a2在I1装配提取后完成内部目录快照/公开V1映射分离，完整制作组对照恢复通过。E1只读边界本切片已验证，但外部请求/写能力未开放；I1完整Campaign/Mission生命周期和其他领域/B1硬预算仍未完成。

[最新HANDOFF/证据](../handoffs/2026-09-15-snapshot-boundary-handoff.md)。下一优先项仍为真实生命周期接缝，不从目录Ready推导游戏可提交，不凭局部通过宣布14类职责全部DONE。

## 以下为原计划与历史进度

# 当前实施增量：I1 装配职责首段已拆（2026-09-15）

生产955a6be3已经执行[框架编排蓝图](../architecture/af-core-composition-blueprint-20260915.md)中的原装配职责提取：36行为/4模型与3组目录声明接回唯一入口。切片离线验证通过，但I1完整生命周期未完成、E1公共执行未开放、B1未整批合格，不能把整个14类计划标DONE。

下一段核实并提取Campaign/Mission建立/失效/释放的真实owner接缝，保留旧存档/晚结果/失败测试；随后继续公共投影分离和本计划领域职责拆分。[最新HANDOFF与已知测试阻断](../handoffs/2026-09-15-composition-extraction-handoff.md)。未推送/部署，自动化暂停。

## 以下为原计划，当前进度以上方为准

# 当前优先级补充：整体框架编排先行（2026-09-15）

用户要求先编排框架，按[装配/生命周期/调用链蓝图](../architecture/af-core-composition-blueprint-20260915.md)先核对唯一装配入口与作用域，再继续下面的14类职责迁移。本轮仅形成蓝图，未改变C#；不据此放行B1或删旧/切默认。
...
- D-A已由用户明确选择Native/Scene/Courier三渠道全部开放，三者均为必交；D-B（任意扩展注册/制作组能力公开）未单独确认，不能借D-A擅自扩大。
- 当前Api.V1只读只是已实现状态，不是本轮已选终态；三渠道真实提交/结果/取消/通知及独立DLL编译+实际加载验收完成前，不得把SDK勾完成。
