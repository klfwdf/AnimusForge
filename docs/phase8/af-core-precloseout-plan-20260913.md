...
版本：v1，2026-09-13。**状态：PLAN_READY / 待用户审阅；除既有基线外，下面工作包尚未开始。**

本轮授权是编写新计划，不是执行全部清单。终点为 `READY_FOR_CLOSEOUT_REVIEW`（可进入收尾评审），不是阶段 8 DONE、已发布或零 BUG。自动化保持暂停；生产、默认路径、游戏目录与真实存档不因本计划而改变。
...
本次仅文档规划。复核了 Git 状态、当前交接、双 Skill、既有功能清单与代码坐标；未重跑游戏构建/测试，未修改生产/Skill/自动化、未推送。批准某个工作项后才把它改为 ACTIVE；其他项保持未开始或待决。
