...
public partial class RewardSystemBehavior
{
        ...
        string giverName)
    {
        ...
        EconomyReplayBatchOutcome outcome = EconomyReplayBatchCoordinator.Execute(
            plan?.Actions,
            action => ExecutePartyEconomyStep(action, giverParty, giverCharacter, receiver, giverName),
            (action, exception) => LogEconomyReplayFailureSafe(
                "[RefactorPartyEconomy] action failed kind=" + action.Kind + " error=" + exception.Message));
        ...
        foreach (EconomyReplayAppliedFact fact in outcome.Facts)
        {
            ...
            catch (Exception exception)
            {
                LogEconomyReplayFailureSafe(
                    "[RefactorPartyEconomy] confirmed fact failed kind=" + fact.Action.Kind + " error=" + exception.Message);
            }
        }
        return EconomyReplayBatchCoordinator.Complete(
            ...
            "economy.party_no_action_applied",
            "economy.party_no_actions",
            "economy.party_partial_replay");
    }
