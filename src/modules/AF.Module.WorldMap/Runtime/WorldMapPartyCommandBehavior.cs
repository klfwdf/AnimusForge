...
public sealed partial class WorldMapPartyCommandBehavior : CampaignBehaviorBase
{
	...
	private readonly WorldMapPendingRequestCoordinator _governorExpeditionRequestTickets = new WorldMapPendingRequestCoordinator();
	...
	public override void SyncData(IDataStore dataStore)
	{
		...
		lock (_queueLock)
		{
			...
			_governorExpeditionRequestTickets.Reset();
	...
	private void StopQueue(Hero hero, string reason, out string fact)
	{
		...
		lock (_queueLock)
		{
			...
			if (_pendingGovernorExpeditionRequests.TryGetValue(hero.StringId, out PendingGovernorExpeditionRequest pendingGovernor))
			{
				_governorExpeditionRequestTickets.TryCancel(hero.StringId, pendingGovernor?.RequestTicket ?? 0L);
			}
	...
	private GovernorExpeditionRecord RemoveGovernorExpeditionRecord(string heroId, string reason, bool removeQueue)
	{
		...
		lock (_queueLock)
		{
			...
			if (_pendingGovernorExpeditionRequests.TryGetValue(key, out PendingGovernorExpeditionRequest pendingGovernor))
			{
				_governorExpeditionRequestTickets.TryCancel(key, pendingGovernor?.RequestTicket ?? 0L);
			}
