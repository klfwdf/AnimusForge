...
- 当前工作树 `E:/AnimusForge-refactor-continuation-20260831`、分支 `codex/af-main-refactor-continuation-20260831`；本轮产品 `34b033de`、`8ca6c27e`、`0bf579f8`、`5bcb518d`、`0a0f7e54`、`eae59e63`、`bda3e6a2`、`3be39594`、`70db6ec2`，测试 `bacaaa92`、`30c817c2`、`30afb8f7`、`8c209163`、`7c7931ae`、`68a4edf5`、`de149e94`，注释 `8f576dc8`。Lore 候选、MCM 数值快照和每版本一次的脱离原对象规则快照、无预选额外规则、直接/称谓实体评分、关系/距离快照与全局实体分配已接到 Native/Courier 的游戏捕获→纯 DTO 后台检索→游戏完成阶段；Scene 仍同步，完整调度归 J10。详细代码责任、验证和具体阻塞见[主台账当前节](docs/animusforge-refactoring-and-repository-reorganization-plan.md#j06-retrieval-cutover-20260919)及[范围图](docs/architecture/af-framework-code-scope.md)；[291 锚点代码地图](docs/architecture/af-framework-code-map.json) recorded/working-tree PASS。
- J03/J04/Knowledge/Courier/Native/Scene 聚焦契约以及 13 个源码边界变异、5 个生产方法负向变异及 Courier 迟到阶段守卫变异已按预期拒收；当前 `70db6ec2` 的 Debug/Release × 1.3/1.4/Bootstrap 六个**非删除性直接构建**均 0 警告/0 错误，不冒充原脚本验收。原 `build_single_module.ps1` 的 Debug 运行因会递归重置四个工作区固定生成目录，被自动审核以“本次缺明确授权”拒绝；没有绕过或尝试 Release。NativePreparation 旧 source-parity 断言另有已记录的既有失败。
- **J06 不标 `OFFLINE_VERIFIED`**：实体分配已迁后台，但新增逐候选关系/距离读取尚缺真实捕获耗时与文本对照；索引缓存/版本和 Knowledge 阶段异常已有生产方法契约，Courier 迟到回放已覆盖；Native 迟到/异常、实体与 Prompt 完整文本对照仍未齐；当前源码原脚本 Debug/Release 六项未跑。实机、旧档、真实 provider `NOT-RUN` **不是**离线验收门槛。未 push、Stage、部署、改 J07/J10 或清理 `.dotnet-cli-home/`。
...
- **J06 仍 `VERIFY / NOT_ACCEPTED`**：导入 8 个纯/文件方法已归 Knowledge，游戏状态校验门面保留；生产方法提取的捕获契约 12 项通过，两项变异预期失败，官方离线构建通过；真实游戏异常/提前评估副作用及 import/save 邻接矩阵仍未实测。实机、旧档、真实 provider `NOT-RUN`。未 push、Stage、部署、打包或改 J07；`.dotnet-cli-home/` 保留。
