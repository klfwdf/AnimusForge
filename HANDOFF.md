...
- **下一步先完成 J08a 上层非流配置/重试/诊断编排的责任划分与模块化，再 J08b 流式、J08c 模型/Gateway、J08d TTS。** 不把本次共享 HTTP attempt 包当作 J08 全部完成，不重开已闭合 J07。双 SKILL 已增加 `llm-transport.md`：一次 attempt、retry owner、资源释放、取消/超时、SSE/Unicode/TTS 边界作为稳定方法，而非写死当前提交。
...
- 自动化 `af-7-8` 已优化为 **AF J08 通宵收尾**，ACTIVE、每 30 分钟接续；新工作窗截止 UTC `2026-09-21T01:00:00Z`（北京时间 09:00）。只做 J08 完整责任包，完成/截止/需外部输入时更新交接并自动暂停；单代理、不自动推送/部署/Stage/打包/改游戏或存档。
