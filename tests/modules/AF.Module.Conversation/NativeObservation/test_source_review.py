...
class RawPresentationSourceTests(unittest.TestCase):
    ...
    def test_side_effects_stay_inside_validated_callback_in_order(self):
        s=projected_source((ROOT/'ShoutBehavior.cs').read_text(encoding='utf-8-sig'));body=ex.declaration(s,'private async Task<string> SubmitNativeConversationTextInternalAsync(')
    ...
    def test_unrelated_or_lost_observer_edits_rejected(self):
        s=projected_source((ROOT/'ShoutBehavior.cs').read_text(encoding='utf-8-sig'));call='SubmitNativeConversationSceneActionObservation(postprocessReply, nativeTargetAgentIndex);'
