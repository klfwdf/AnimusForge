"""Build and run the Knowledge entity retrieval owners contract (fuzzy matcher / mention list / injection allocator) against the production owner files.
...
if args.mutate:
    ...
    src_dir.mkdir()
    rel = "src/modules/AF.Module.Knowledge/Entities/EntityInjectionAllocator.cs"
    text = (ROOT / rel).read_text(encoding="utf-8")
    if args.mutate == "scope-boost-off":
        needle = "score += 1000 + priorityBonus;"
        replacement = "score += 0;"
    else:
        needle = "addedForMention >= MaxSecondaryMatchesPerMention"
        replacement = "false"
...
(output / "KnowledgeEntitiesTests.csproj").write_text(project, encoding="utf-8")
...
build = subprocess.run([str(dotnet), "build", str(output / "KnowledgeEntitiesTests.csproj"), "-c", "Release", "--nologo", "-p:RestoreConfigFile=" + str(output / "NuGet.Config")], cwd=output, env=env, capture_output=True, text=True, encoding="utf-8", errors="replace")
...
run = subprocess.run([str(dotnet), str(output / "bin/Release/net8.0/KnowledgeEntitiesTests.dll")], cwd=ROOT, env=env, capture_output=True, text=True, encoding="utf-8", errors="replace")
